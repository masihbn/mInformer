...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.test_set_length is not None:
            ...
            num_vali = self.test_set_length / 2
            num_train = len(df_raw) - num_vali - num_test
        else:
            num_train = int(len(df_raw)*0.7)
            num_test = int(len(df_raw)*0.2)
            num_vali = len(df_raw) - num_train - num_test
        border1s = [0, int(num_train-self.seq_len), int(len(df_raw)-num_test-self.seq_len)]
        border2s = [int(num_train), int(num_train+num_vali), len(df_raw)]
        ...
        if self.scale:
            candle_colors = df_data['color']
            train_data = df_data[border1s[0]:border2s[0]].drop(['color'], axis=1)
            self.scaler.fit(train_data.values)
            tmp_data = self.scaler.transform(df_data.drop(['color'], axis=1).values)
            data = pd.concat([pd.DataFrame(tmp_data), candle_colors], axis=1, join='inner').to_numpy()
        else:
            data = df_data.values
    ...
    def __getitem__(self, index):
        ...
        r_begin = s_end - self.seq_len
        r_end = r_begin + self.seq_len + 1
